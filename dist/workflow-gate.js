 ...
 */
export async function buildNonAdvancingNoteNotice(intent, issueId, authToken, callerLinearUserId) {
    if (intent !== "note")
        return null;
    if (!issueId)
        return null;
    const { labels, delegateId, fetchFailed } = await fetchTicketContext(issueId, authToken);
    if (fetchFailed)
        return null; // fail-open: can't confirm a violation
    const workflowId = getWorkflowId(labels);
    if (!workflowId)
        return null; // ad-hoc ticket — note is not a workflow turn
    ...
    if (currentState.kind === "terminal" || !currentState.transitions?.length)
        return null;
    const breakGlassCommand = def.break_glass?.command ?? "escape";
    ...
    helpLines.push(`  - \`linear ${breakGlassCommand} ${issueId}\` (break glass, legal from any state)`);
    return (`[Workflow] Your comment was posted, but it did **not** advance the ticket. ` +
