 ...
 */
export async function loadWorkflowDefById(workflowId) {
    const registry = await loadWorkflowRegistry();
...
export function validateNativeStateMappings(def) {
    ...
    for (const state of def.states) {
        ...
        if (!VALID_SEMANTIC_STATES.has(state.native_state)) {
            warnings.push(`Workflow state '${state.id}' has native_state '${state.native_state}' which is not a recognized semantic state. ` +
 ...
 */
async function fetchIssueDescription(issueId, authToken) {
 ...
 */
export async function resolveMetaIntent(intent, issueId, authToken) {
    ...
    const genericRole = intent === 'continue-workflow' ? 'continue' : 'revision';
    ...
    if (!transition) {
        ...
        return {
            error: `[Proxy] '${intent}' has no ${genericRole} transition in state '${currentState}' ` +
...
export async function checkWorkflowRules(intent, issueId, authToken, bodyId, target = null, callerLinearUserId, artifactRef, breakGlassOverride = false, isMetaIntent = false) {
    ...
    if (fetchFailed && !breakGlassOverride) {
        ...
        if (looksLikeWorkflowCommand) {
            ...
            return (`[Proxy] '${intent}' blocked: unable to fetch ticket context for ${issueId}. ` +
    ...
    if (!breakGlassOverride && !isConfigHealthy()) {
        ...
        return (`[Proxy] '${intent}' blocked: config artifacts are degraded and enforcement cannot be trusted. ` +
    ...
    if (intent === breakGlassCommand) {
        ...
        return (`[Proxy] 'escape' blocked: '${bodyId}' is not the current delegate or the workflow steward. ` +
...
export async function checkRawMutationInterception(body, issueId, authToken, bodyId, callerLinearUserId, skipCommentCreate) {
    ...
    if (!isIssueUpdate && !isCommentCreate)
        return null;
    ...
    if (skipCommentCreate && isCommentCreate)
        return null;
    ...
    if (isCommentCreate) {
        if (!issueId) {
            ...
            return (`[Proxy] Direct commentCreate on workflow tickets must go through workflow commands, ` +
        ...
        if (!ccState) {
            ...
            for (const s of ccDef.states) {
                for (const t of s.transitions ?? [])
                    allCmds.add(t.command);
            }
            allCmds.add(ccBreakGlass);
            return (`[Proxy] Direct commentCreate is blocked on this workflow ticket (state unknown). ` +
                `Use workflow commands instead: ${[...allCmds].join(", ")}.`);
        }
        const ccStateNode = ccDef.states.find((s) => s.id === ccState);
        if (!ccStateNode)
            return null;
        const ccHelpLines = await Promise.all((ccStateNode.transitions ?? []).map(async (t) => {
        ...
        ccHelpLines.push(`  - \`linear ${ccBreakGlass} ${issueId}\` (break glass → ${ccDef.break_glass?.to ?? "escape"}, legal from any state)`);
        return (`[Proxy] Direct commentCreate is blocked on this workflow ticket ` +
    ...
    const queryHasField = (field) => new RegExp(`\\b${field}\\b`).test(q);
    const varsHaveKey = (obj, key) => {
        if (!obj || typeof obj !== "object")
            return false;
        if (Array.isArray(obj))
            return obj.some((v) => varsHaveKey(v, key));
        const rec = obj;
        if (key in rec)
            return true;
        return Object.values(rec).some((v) => varsHaveKey(v, key));
    };
    const touches = (field) => queryHasField(field) || varsHaveKey(vars, field);
    ...
    if (!issueId) {
        ...
        return (`[Proxy] Direct status/assignee/label changes on workflow tickets must go through ` +
...
export async function applyStateTransition(intent, issueId, authToken, options) {
    ...
    if (currentStateName === toStateName) {
        ...
        if (hasTargetLabel) {
            log.info(`workflow-gate: B2 apply: ${issueId} already in state '${toStateName}' with label present — no-op`);
            return;
        }
        // Label is missing despite being in the correct state — re-stamp.
        log.warn(`workflow-gate: B2 apply: ${issueId} is in state '${toStateName}' but label '${targetLabelName}' is missing — re-stamping`);
        const newLabelId = await findOrCreateLabel(issue.teamId, targetLabelName, authToken);
        if (!newLabelId) {
        ...
        if ((intent === 'reject' || intent === 'request-changes' || intent === 'ac-fail') && toStateName === 'implementation') {
            ...
            if (priorImplementer) {
                ...
                if (agent?.linearUserId) {
                    ...
                    log.info(`workflow-gate: B2 apply: ${issueId} ${intent} → implementation, routing to prior implementer '${priorImplementer}'`);
                }
                else {
                    log.error(`workflow-gate: B2 apply: FAIL-CLOSED — prior implementer '${priorImplementer}' has no linearUserId. Cannot route ${intent} on ${issueId}.`);
                    return;
                }
            }
            else {
                log.warn(`workflow-gate: B2 apply: no prior implementer recorded for ${issueId} on ${intent} — falling back to role resolution`);
            }
        }
            ...
            try {
                ...
                if (roleBodies.length === 1) {
                    ...
                    if (intent === 'approve' || intent === 'reject') {
                        log.error(`workflow-gate: B2 apply: FAIL-CLOSED — no bodies found for role '${destOwnerRole}' on '${intent}'. Transition aborted per AI-1493.`);
                        return;
                    }
                    log.warn(`workflow-gate: B2 apply: no bodies found for role '${destOwnerRole}' on '${intent}' — skipping auto-delegate`);
                }
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                log.error(`workflow-gate: B2 apply: FAIL-CLOSED — role resolution failed for '${destOwnerRole}': ${msg}. Transition aborted.`);
                return;
            }
        }
    }
    ...
    if (toStateName === 'implementation' && resolvedDelegateId) {
        ...
        if (implementerAgent) {
            ...
            log.info(`workflow-gate: B2 apply: recorded implementer '${implementerAgent.name}' for ${issueId}`);
        }
 ...
 */
async function issueUpdateDelegateOnly(internalId, delegateLinearUserId, authToken) {
    const mutation = `
