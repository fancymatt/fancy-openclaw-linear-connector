...
function acknowledgeAgentAuthoredActivity(event, onAgentActivity) {
    ...
    if (event.type !== "Comment" && event.type !== "AgentSessionEvent")
        return;
...
export function createWebhookRouter(eventStore, nudgeStore, agentQueue, bag, sessionTracker, throttle, operationalEventStore, onDispatched, onAgentActivity, onDeliveryCommitted) {
    ...
    router.post("/", async (req, res) => {
        ...
        if (event.type === "Issue") {
            ...
            const enrollData = event.data;
            const enrollIssueId = enrollData?.id;
            if (enrollToken && enrollIssueId) {
                const enrollIdentifier = enrollData?.identifier ?? enrollIssueId;
                ...
                if (bootstrapResult) {
                    ...
                    if (bootstrapResult.action === "bootstrapped" && bootstrapResult.delegateAgentName && bootstrapResult.ticketIdentifier) {
                        ...
                        try {
                            ...
                            log.info(`Bootstrap wake delivered to ${bootstrapResult.delegateAgentName} for ${bootstrapResult.ticketIdentifier} (runId=${wakeResult.runId ?? "ok"})`);
                            appendOperationalEvent(operationalEventStore, {
                            ...
                            if (onDispatched)
                                onDispatched(bootstrapResult.delegateAgentName, wakeSessionKey);
                        }
                        catch (err) {
                            log.error(`Bootstrap wake delivery failed for ${bootstrapResult.delegateAgentName}: ${err instanceof Error ? err.message : String(err)}`);
                            appendOperationalEvent(operationalEventStore, {
