 ...
 */
export function classifyTicket(labels, delegateId, workflowDef, roleBodiesForRole) {
    ...
    if (!stateDef?.owner_role)
        return "healthy"; // no role constraint → healthy by default
    const validBodies = roleBodiesForRole(stateDef.owner_role);
    if (validBodies.includes(delegateId))
        return "healthy";
    return "drifted";
}
// ── Capability policy loading ──────────────────────────────────────────────
function loadCapabilityPolicy(policyPath) {
    try {
...
function buildRoleResolver(policy) {
    if (!policy?.bodies)
        return () => [];
    const bodies = policy.bodies;
    return (roleId) => bodies.filter((b) => b.fills_roles?.includes(roleId)).map((b) => b.id);
}
// ── Linear API helpers ─────────────────────────────────────────────────────
async function fetchWfTickets(authToken) {
    const query = `
...
async function setDelegate(ticketId, delegateId, authToken) {
    const mutation = `
 ...
 */
export async function runRescueSweep(options) {
    const { authToken, workflowRegistry = new Map(), capabilityPolicyPath, operationalEventStore, labelNameToId: injectedLabelNameToId, } = options;
    const errors = [];
    const rescues = [];
    const byClassification = {};
    const rescuedIds = new Set(); // idempotency guard within this sweep
    ...
    const resolvedPolicyPath = capabilityPolicyPath ??
    ...
    for (const ticket of tickets) {
        ...
        if (classification === "terminal" || classification === "healthy")
            continue;
        // Within-sweep idempotency: skip if already rescued this run
        if (rescuedIds.has(ticket.id))
            continue;
        let rescueAction;
...
// ── Rescue helpers ─────────────────────────────────────────────────────────
async function rescueMalformed(ticket, wfDef, roleBodiesForRole, labelNameToId, authToken) {
...
async function rescueDormant(ticket, wfDef, roleBodiesForRole, authToken) {
...
async function rescueDrifted(ticket, wfDef, roleBodiesForRole, authToken) {
    const stateId = ticket.labels.find((l) => l.startsWith("state:"))?.slice("state:".length);
    const stateDef = stateId ? wfDef.states.find((s) => s.id === stateId) : undefined;
    const ownerRole = stateDef?.owner_role;
    const candidates = ownerRole ? roleBodiesForRole(ownerRole) : [];
...
//# sourceMappingURL=rescue-sweep.js.map