...
function agentAnchor(agentName) {
    return `agent-${slugId(agentName)}`;
}
function oauthAnchor(agentName) {
    return `oauth-${slugId(agentName)}`;
}
function taskAnchor(sessionKey) {
...
function safeAgent(agent, queueStat, pendingCount = 0, activeSessionKey) {
    ...
    const severity = !bool(agent.linearUserId) || !credentialComplete
        ? "red"
...
function safeTaskFromRoute(route, state, updatedAt) {
...
function safeTaskFromBag(entry) {
    return {
        owner: entry.agentId,
        agent: entry.agentId,
        state: "pending",
...
function buildDashboard(deps) {
    ...
    const agentRows = agents.map((agent) => {
        ...
        const row = safeAgent(agent, queueStats.find((s) => s.agentId === agent.name || s.agentId === openclawAgent), pendingStats.find((s) => s.agentId === agent.name || s.agentId === openclawAgent)?.pendingCount ?? 0, deps.sessionTracker.getActiveSessionKey(agent.name) ?? deps.sessionTracker.getActiveSessionKey(openclawAgent));
    ...
    const agentIds = (agent) => [...new Set([agent.name, agent.openclawAgent ?? agent.name])];
...
function renderAuthFailurePage(status, title, message) {
...
function sendAdminAuthFailure(req, res, status, title, message) {
    if (wantsHtml(req)) {
        if (status === 401)
...
function attentionHtml(dashboard) {
...
function agentTableHtml(rows) {
    if (rows.length === 0)
        return `<div class="empty">No agents configured.</div>`;
...
function taskTableHtml(rows) {
    if (rows.length === 0)
        return `<div class="empty">No work in this state.</div>`;
...
function renderAgentsPage(dashboard) {
...
function renderTasksPage(dashboard) {
...
function renderSettingsPage(dashboard) {
...
function renderShell(initialPage, dashboard) {
    const nav = [
