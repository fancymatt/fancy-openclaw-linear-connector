...
const STAKE_LABELS = [null, 'stakes:low', 'stakes:medium', 'stakes:high'];
const CALLER_KINDS = [
    'delegate',
...
function resolveLinearUserId(bodyId) {
...
function bodiesForRole(policy, role) {
    return policy.bodies.filter((b) => b.fills_roles.includes(role)).map((b) => b.id);
}
function bodyHasCap(policy, bodyId, cap) {
 ...
 */
function buildCallerSpec(policy, stateOwnerRole, kind) {
 ...
 */
function resolveExpected(stateNode, command, spec, stakeLabel, def, policy, breakGlassCommand) {
    ...
    if (command === breakGlassCommand) {
        if (!spec.delegateLinearUserId)
            return { expected: 'allow' };
        if (!spec.linearUserId)
            return { expected: 'allow' };
...
export function buildConformanceMatrix(def, policy) {
    ...
    for (const stateNode of def.states) {
        ...
        const specsForState = new Map(CALLER_KINDS.map((k) => [k, buildCallerSpec(policy, stateNode.owner_role, k)]));
        for (const command of commandList) {
            for (const kind of CALLER_KINDS) {
                const spec = specsForState.get(kind);
                if (!spec)
                    continue; // no representative body for this kind in this state
                for (const stakeLabel of STAKE_LABELS) {
                    const outcome = resolveExpected(stateNode, command, spec, stakeLabel, def, policy, breakGlassCommand);
                    const cell = {
                        state: stateNode.id,
                        command,
                        caller: {
                    ...
                    if (outcome.blockReason)
                        cell.blockReason = outcome.blockReason;
                    if (outcome.requiredCapability)
                        cell.requiredCapability = outcome.requiredCapability;
                    cells.push(cell);
                }
            }
        }
...
//# sourceMappingURL=conformance-matrix.js.map