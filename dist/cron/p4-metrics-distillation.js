...
async function proposalExists(name) {
    try {
        const result = await invokeGatewayTool("skill_workshop", { action: "list" });
        if (!result || !Array.isArray(result.proposals))
            return false;
        const proposals = result.proposals;
        return proposals.some((p) => p.name === name && (!p.status || p.status === "pending"));
    }
    catch (err) {
...
function buildProposalContent(workflow, step, reasonCode, count, description) {
    return [
...
function buildDescription(workflow, step, reasonCode, count) {
    const reasonLabels = {
 ...
 */
export async function runDistillation(observationStore, threshold) {
 ...
 */
export function registerDistillationCron(observationStore) {
...
//# sourceMappingURL=p4-metrics-distillation.js.map