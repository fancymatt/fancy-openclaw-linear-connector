...
import { checkWorkflowRules, checkRawMutationInterception, applyStateTransition, buildStateTransitionReminder, buildNonAdvancingNoteNotice, fetchWorkflowLabels, fetchTeamStateLabelIds, getCurrentState } from "./workflow-gate.js";
...
async function runWithTicketLock(ticketId, fn, onConflict) {
 ...
 */
function minWorkflowCliVersion() {
 ...
 */
function isIssueUpdateMutation(body) {
 ...
 */
function issueUpdateInput(body) {
 ...
 */
function carriesLabelDelta(body) {
    const input = issueUpdateInput(body);
    if (!input)
        return false;
    return ["addedLabelIds", "removedLabelIds"].some((k) => Array.isArray(input[k]) && input[k].length > 0);
}
function stripStateLabelDeltas(body, stateLabelIds) {
    if (!body || stateLabelIds.size === 0)
        return 0;
    const input = issueUpdateInput(body);
    if (!input)
        return 0;
...
export async function handleProxyRequest(req, res, deps) {
    ...
    const artifactRefHeader = req.headers["x-openclaw-artifact-ref"] ?? null;
    const cliVersion = req.headers["x-openclaw-linear-cli-version"] ?? null;
