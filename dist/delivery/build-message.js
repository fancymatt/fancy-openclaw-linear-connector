...
import { loadWorkflowDefById, fetchWorkflowLabels, TransientLabelFetchError, getWorkflowId, getCurrentState, resolveTransitionTargets, resolveStakesLevel, } from "../workflow-gate.js";
 ...
 */
async function fetchLabelsWithRetry(identifier, authToken, maxRetries = parseInt(process.env.LABEL_FETCH_MAX_RETRIES ?? "2", 10), baseDelayMs = parseInt(process.env.LABEL_FETCH_BASE_DELAY_MS ?? "500", 10)) {
    let lastError;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (err instanceof TransientLabelFetchError && attempt < maxRetries) {
                ...
                log.warn(`build-message: label fetch attempt ${attempt + 1}/${maxRetries + 1} failed for ${identifier} (${err.message}) — retrying in ${delayMs}ms`);
...
export async function buildWorkflowAwareDeliveryMessage(identifier, authToken, actionText = `You have a pending ticket: ${identifier}`) {
    ...
    try {
        ...
        if (res.status === 401 || res.status >= 500) {
            log.warn(`build-message: title fetch for ${identifier} returned ${res.status} — proceeding with fallback`);
        }
        else {
        ...
        log.warn(`build-message: title fetch failed for ${identifier}: ${reason} — proceeding with fallback`);
    }
 ...
 */
export async function tryBuildWorkflowMessage(actionText, identifier, title, authToken) {
    let labels;
    try {
        ...
        const reason = err instanceof Error ? err.message : String(err);
        log.warn(`build-message: workflow label fetch exhausted retries for ${identifier} — delivering workflow-context-unavailable fallback. Failure reason: ${reason}`);
 ...
 */
function buildWorkflowContextUnavailableMessage(actionText, identifier, title) {
    return [
        `${actionText}: ${title}`,
        "",
