...
export class DispatchAckTracker {
    constructor(dbPath, ttlMs) {
        const resolvedPath = dbPath ?? path.join(process.cwd(), "data", "dispatch-acks.db");
     ...
     */
    recordDispatch(agentId, ticketId) {
     ...
     */
    acknowledge(agentId, ticketId) {
        if (ticketId) {
            const normalizedId = normalizeSessionKey(ticketId);
            const result = this.db
                .prepare(`UPDATE dispatch_acks SET ack_status = 'acknowledged'
     ...
     */
    markResignaled(agentId, ticketId) {
        const normalizedId = normalizeSessionKey(ticketId);
        this.db
            .prepare(`UPDATE dispatch_acks
     ...
     */
    markEscalated(agentId, ticketId) {
        const normalizedId = normalizeSessionKey(ticketId);
        this.db
            .prepare(`UPDATE dispatch_acks SET ack_status = 'escalated'
         WHERE agent_id = ? AND ticket_id = ?`)
            .run(agentId, normalizedId);
     ...
     */
    getDeferredStale(staleMs) {
        const cutoff = new Date(Date.now() - staleMs)
            .toISOString()
            .replace("T", " ")
            .replace(/\.\d{3}Z$/, "");
        const rows = this.db
            .prepare(`SELECT id, agent_id, ticket_id, dispatched_at, last_signal_at,
                ack_status, attempt_count
         FROM dispatch_acks
     ...
     */
    hasRecentPending(agentId, ticketId, withinMs) {
        const normalizedId = normalizeSessionKey(ticketId);
        const cutoff = new Date(Date.now() - withinMs)
            ...
            .replace(/\.\d{3}Z$/, "");
        const row = this.db
            .prepare(`SELECT 1 FROM dispatch_acks
         WHERE agent_id = ? AND ticket_id = ?
           AND ack_status IN ('pending', 'unconfirmed')
           AND dispatched_at >= ?
         LIMIT 1`)
    ...
    cleanup() {
        const cutoff = new Date(Date.now() - this.ttlMs)
            ...
            .replace(/\.\d{3}Z$/, "");
        const result = this.db
            .prepare(`DELETE FROM dispatch_acks
...
//# sourceMappingURL=dispatch-ack-tracker.js.map