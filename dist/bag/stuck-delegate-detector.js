 ...
 */
export function buildRePrompt(ticketId, currentState, def) {
    ...
    if (!stateNode || !stateNode.transitions?.length) {
        ...
        log.warn(`Stuck-delegate: buildRePrompt reached terminal/unknown state fallback for ${ticketId} ` +
        ...
        return (`[Stuck-delegate detection] Ticket ${ticketId} is in state '${currentState}' but appears stuck. ` +
    ...
    return (`[Stuck-delegate detection] You posted a completion comment but ticket ${ticketId} is still ` +
...
export class StuckDelegateDetector {
    ...
    start() {
        if (this.timer)
            return;
        log.info(`Stuck-delegate detector started — poll=${this.config.pollMs}ms ` +
            `idleGrace=${this.config.idleGraceMs}ms maxPrompts=${this.config.maxPrompts}`);
        this.timer = setInterval(() => {
            this.runCycle().catch((err) => {
                log.error(`Stuck-delegate cycle error: ${err instanceof Error ? err.message : String(err)}`);
            });
        }, this.config.pollMs);
        this.timer.unref();
    }
    ...
    async runCycle() {
        ...
        for (const agent of agents) {
            ...
            try {
                ...
                result.errors++;
                log.error(`Stuck-delegate fetch failed for ${agent.name}: ${err instanceof Error ? err.message : String(err)}`);
                continue;
            }
            for (const candidate of candidates) {
                ...
                try {
                    ...
                    if (sent) {
                        ...
                        log.info(`Stuck-delegate: re-prompted ${openclawAgent} for ${ticketId} (prompt ${newCount}/${this.config.maxPrompts})`);
                    ...
                    result.errors++;
                    log.error(`Stuck-delegate wake failed for ${openclawAgent} / ${ticketId}: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        }
        return result;
 ...
 */
function defaultSendWakeFactory(deliveryConfig) {
    return async (agentOpenclawName, ticketId, prompt) => {
        const sessionKey = normalizeSessionKey(ticketId);
        ...
        try {
            ...
            if (!result.dispatched) {
                log.error(`Stuck-delegate wake delivery failed for ${agentOpenclawName} / ${ticketId}: ` +
            ...
            log.error(`Stuck-delegate wake delivery threw for ${agentOpenclawName} / ${ticketId}: ${err instanceof Error ? err.message : String(err)}`);
            return false;
        }
    };
}
 ...
 */
async function defaultFetchStuckCandidates(agent) {
  ...
  `;
    try {
        ...
        log.error(`Failed to fetch stuck candidates for ${agent.name}: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
//# sourceMappingURL=stuck-delegate-detector.js.map