...
const SESSION_DEDUP_WINDOW_MS = 30_000; // 30 seconds
 ...
 */
export async function createSessionAndEmitThought(
  issueId: string,
  agentName: string,
  issueContext?: { identifier?: string; title?: string; description?: string },
...
async function doCreateSessionAndEmitThought(
  issueId: string,
  agentName: string,
  issueContext?: { identifier?: string; title?: string; description?: string },
): Promise<SessionCreateResult> {
  const token = getAccessToken(agentName);
  if (!token) return { sessionId: null, result: `session skipped: no token for ${agentName}` };
  ...
  try {
    ...
    if (!sessionId) return { sessionId: null, result: "session created but no id returned" };
