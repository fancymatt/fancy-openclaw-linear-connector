...
function buildDashboard(deps: AdminDeps) {
  ...
  for (const agent of agentRows) {
    if (!agent.identityMapped || !agent.oauthConfigured) {
      attention.push({ severity: "red", title: `${agent.name} setup incomplete`, message: "Identity mapping or OAuth credentials are missing. Secrets are hidden; use the onboarding flow to repair setup.", href: "/admin/agents" });
    } else if (agent.pendingCount > 0 || agent.queueDepth > 0) {
      attention.push({ severity: "yellow", title: `${agent.name} has work waiting`, message: `${agent.pendingCount + agent.queueDepth} pending/queued item(s) need agent pickup or session completion.`, href: "/admin/tasks" });
    }
  }
...
function attentionHtml(dashboard: ReturnType<typeof buildDashboard>): string {
  return `<section class="card span-12 attention"><h2>Attention Needed</h2>${dashboard.attention.length
...
function taskTableHtml(rows: ReturnType<typeof buildDashboard>["tasks"]): string {
  ...
  return `<table><thead><tr><th>Task</th><th>Owner / Agent</th><th>State</th><th>Lifecycle context</th></tr></thead><tbody>${rows.map((task) => `<tr><td><div class="row-title">${task.relatedUrl ? `<a href="${escapeHtml(task.relatedUrl)}">${escapeHtml(task.related)}</a>` : escapeHtml(task.related)}</div><div class="muted">${escapeHtml(task.eventType)} · ${escapeHtml(task.sessionKey)} · ${escapeHtml(task.age)} · updated ${escapeHtml(ageLabel(task.updated))}</div></td><td>${escapeHtml(task.owner)}</td><td>${chip(task.severity as Severity, task.state)}${task.safeError ? `<div class="muted">${escapeHtml(task.safeError)}</div>` : ""}</td><td>${escapeHtml(task.lifecycle)}${diagnostics(task.diagnostics)}</td></tr>`).join("")}</tbody></table>`;
}
