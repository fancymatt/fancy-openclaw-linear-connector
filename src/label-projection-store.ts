/**
 * AI-1488: Label-projection store.
 *
 * Records the authoritative connector-side workflow state for each governed
 * ticket. This is the "store" in the store-keyed detection design (§4.2):
 *
 ...
 *     `unlabel wf:*` is drift (auto-reconciled + alerted), never an escape hatch.
 *   - On ticket exit (escape, __ad_hoc__) the record is removed.
 *
...
export async function recordProjection(issueId: string, workflowId: string, stateName: string): Promise<void> {
  await ensureLoaded();
