...
const artifactState: Record<ArtifactKind, ArtifactHealth> = {
  "workflow-def": {
    kind: "workflow-def",
    healthy: true,
    lastSuccess: null,
 ...
 */
export function recordSuccess(kind: ArtifactKind): void {
 ...
 */
export function recordFailure(kind: ArtifactKind, error: string): void {
  const artifact = artifactState[kind];
 ...
 */
export function getStatus(): ConfigHealthStatus {
  return {
    healthy: isHealthy(),
 ...
 */
export function resetConfigHealth(): void {
  for (const key of Object.keys(artifactState) as ArtifactKind[]) {
    artifactState[key] = {
      kind: key,
      healthy: true,
      lastSuccess: null,
...
function fireAlerts(status: ConfigHealthStatus): void {
