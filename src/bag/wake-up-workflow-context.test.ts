/**
 * Tests for AI-1669 — Include workflow context in connector wake-up delivery message (v2).
 *
...
describe("AC1 — governed single-ticket delivery message includes state name and legal verb set", () => {
  it("includes the current state name in the message", async () => {
    ...
    const msg = await buildWorkflowAwareWakeUpMessage(["AI-1669"], "Bearer tok");
  ...
  it("strips linear- session-key prefix from ticket ID in the message", async () => {
    ...
    const msg = await buildWorkflowAwareWakeUpMessage(["linear-AI-1669"], "Bearer tok");
...
describe("AC2 — non-governed ticket wake-ups are unchanged", () => {
  ...
  it("multi-ticket (2 tickets) → unchanged generic multi-ticket output", async () => {
    ...
    const msg = await buildWorkflowAwareWakeUpMessage(["AI-1669", "AI-1670"], "Bearer tok");
    const expected = buildWakeUpMessage(["AI-1669", "AI-1670"]);
  ...
  it("no authToken → returns generic message without attempting fetch", async () => {
    ...
    const msg = await buildWorkflowAwareWakeUpMessage(["AI-1669"]);
    const expected = buildWakeUpMessage(["AI-1669"]);
...
describe("AC3 — verb list is derived from the workflow YAML, not hardcoded", () => {
  it("write-tests state lists tests-ready (from dev-impl YAML), not submit or approve", async () => {
    ...
    expect(msg).not.toContain("linear submit AI-1669");
    expect(msg).not.toContain("linear approve AI-1669");
  });
...
states:
  - id: special
    ...
    native_state: todo
    transitions:
      - command: unique-custom-verb-ai1669
        to: done
        assign: { mode: auto }
  ...
  - id: escape
    ...
    native_state: invalid
`;
    const customYamlPath = path.join(tmpDir, "custom-ai1669.yaml");
    ...
    expect(msg).toContain("unique-custom-verb-ai1669");
    expect(msg).not.toContain("tests-ready");
