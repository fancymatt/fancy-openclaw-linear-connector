...
export interface StuckDelegateCycleResult {
  ...
  /** Candidates skipped because a recent pending dispatch ack suggests the session is still active. */
...
export class StuckDelegateDetector {
  ...
  private deps: Required<Omit<StuckDelegateDeps, "ackTracker">> & { ackTracker?: DispatchAckTracker };
  ...
  async runCycle(): Promise<StuckDelegateCycleResult> {
    const { listAgents, fetchStuckCandidates, loadDef, sessionTracker, bag, operationalEventStore, sendWake, ackTracker, now: getNow } = this.deps;
