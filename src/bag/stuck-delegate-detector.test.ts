...
describe("StuckDelegateDetector", () => {
  ...
  describe("runCycle", () => {
    ...
    test("AI-1650: skips when ack tracker has recent pending dispatch", async () => {
      const { bag, sessionTracker, operationalEventStore, deliveryConfig } = setupDeps(dir);
      ...
      const candidates: StuckCandidate[] = [
        {
          ...
          labels: ["wf:dev-impl", "state:implementation"],
          delegateId: TEST_AGENT.linearUserId,
          stateEnteredAt: "2026-06-22T10:00:00.000Z",
          delegateComments: [
            { id: "c1", createdAt: "2026-06-22T10:05:00.000Z", body: "RepoResearcher done" },
          ],
          transitionsAfterEntry: [],
        },
      ];

      // Mock ackTracker: hasRecentPending returns true → session is still running
      const mockAckTracker = {
        hasRecentPending: (_agentId: string, _ticketId: string, _withinMs: number) => true,
      } as unknown as import("./dispatch-ack-tracker.js").DispatchAckTracker;
      ...
      const detector = new StuckDelegateDetector(
        {
          ...
          fetchStuckCandidates: async () => candidates,
          loadDef: async () => TEST_WORKFLOW_DEF,
          sendWake: async (agent, ticket) => { wakeCalls.push(`${agent}:${ticket}`); return true; },
        },
      ...
      // Should skip — recent pending dispatch signals session still active
    ...
    test("AI-1650: allows re-prompt when no pending dispatch in ack tracker", async () => {
      const { bag, sessionTracker, operationalEventStore, deliveryConfig } = setupDeps(dir);
      const wakeCalls: string[] = [];
      ...
      const candidates: StuckCandidate[] = [
        {
          ...
          stateEnteredAt: "2026-06-22T10:00:00.000Z",
          delegateComments: [
            { id: "c1", createdAt: "2026-06-22T10:05:00.000Z", body: "done" },
          ],
          transitionsAfterEntry: [],
        },
      ];

      // Mock ackTracker: hasRecentPending returns false → no recent dispatch
      const mockAckTracker = {
        hasRecentPending: (_agentId: string, _ticketId: string, _withinMs: number) => false,
      } as unknown as import("./dispatch-ack-tracker.js").DispatchAckTracker;
      ...
      // No recent pending dispatch → should re-prompt normally
    ...
    test("AI-1650: allows re-prompt when pending dispatch exceeds threshold", async () => {
      ...
      const candidates: StuckCandidate[] = [
        {
          ...
          labels: ["wf:dev-impl", "state:implementation"],
          delegateId: TEST_AGENT.linearUserId,
          stateEnteredAt: "2026-06-22T09:00:00.000Z",
          delegateComments: [
            { id: "c1", createdAt: "2026-06-22T09:05:00.000Z", body: "done" },
          ],
          transitionsAfterEntry: [],
        },
      ];

      // Mock ackTracker: hasRecentPending returns false → dispatch is older than threshold
      const mockAckTracker = {
        hasRecentPending: (_agentId: string, _ticketId: string, _withinMs: number) => false,
      } as unknown as import("./dispatch-ack-tracker.js").DispatchAckTracker;
      ...
      // Dispatch exceeded threshold → session is done → should re-prompt
      expect(result.skippedSessionActive).toBe(0);
      expect(result.rePromptsSent).toBe(1);
      expect(wakeCalls).toHaveLength(1);
    ...
    test("AI-1650: backward compat — works without ackTracker", async () => {
      ...
      // No ackTracker → guard is skipped, normal re-prompt behavior
      expect(result.skippedSessionActive).toBe(0);
      expect(result.rePromptsSent).toBe(1);
      expect(wakeCalls).toHaveLength(1);
    ...
    test("AI-1650: sessionActiveThresholdMs=0 disables the ack guard", async () => {
      const { bag, sessionTracker, operationalEventStore, deliveryConfig } = setupDeps(dir);
      const wakeCalls: string[] = [];
      const ackTrackerCalls: string[] = [];
      ...
      // Mock ackTracker that would return true (skip) if consulted
      const mockAckTracker = {
        hasRecentPending: (agentId: string, ticketId: string) => {
          ackTrackerCalls.push(`${agentId}:${ticketId}`);
          return true; // would skip if guard were active
        },
      } as unknown as import("./dispatch-ack-tracker.js").DispatchAckTracker;
      ...
      // Guard disabled — ackTracker must not be consulted, re-prompt fires
      expect(ackTrackerCalls).toHaveLength(0);
      expect(result.skippedSessionActive).toBe(0);
      expect(result.rePromptsSent).toBe(1);
      expect(wakeCalls).toHaveLength(1);
