...
// Local alias — the forensic functions accept whatever the tracker produces
...
export type StaleClass =
  | "C1"  // Waiting on user (legitimate) — agent should have used needs-human
  | "C2"  // Tool hang — waiting on tool response that never came
  | "C3"  // Silent completion — finished work but didn't transition
  | "C4"  // Never started — 0 tool calls, 0 text
  | "C5"  // Looped / runaway — many tool calls, no productive output
  | "C6"  // Errored — model error, not recovered
  | "C-UNK"; // Unknown

export const STALE_CLASS_NAMES: Record<StaleClass, string> = {
  C1: "Waiting on user",
  C2: "Tool hang",
...
export interface StaleSnapshot {
  ...
  errors: string[];
  diagnosticPath: string;       // where the snapshot JSON was written
}

// ── Stale session entry (from session-tracker) ──────────────────────────────

// Re-exported from session-tracker.ts — see above
 ...
 */
function readSessionJsonl(filePath: string): JsonlEvent[] {
  try {
...
export function buildSnapshot(
  ...
  const linearTicket: LinearTicketSnapshot = {
...
function extractLastAssistantMessage(events: JsonlEvent[]): LastAssistantMessage | null {
...
function normalizeStopReason(raw?: string): "end_turn" | "tool_use" | "max_tokens" | "error" | "unknown" {
  if (!raw) return "unknown";
  const lower = raw.toLowerCase();
  if (lower === "stop" || lower === "end_turn") return "end_turn";
  if (lower === "tool_use" || lower === "tooluse" || lower === "tool_call") return "tool_use";
  if (lower.includes("max_token") || lower.includes("length")) return "max_tokens";
  if (lower === "error" || lower.includes("error")) return "error";
  return "unknown";
}
...
function extractToolCallSummary(events: JsonlEvent[]): ToolCallSummary {
  const byName: Record<string, number> = {};
  const allCalls: ToolCallEntry[] = [];
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    ...
    for (const call of allCalls) {
      if (call.result !== "no-result") continue;
...
function extractErrors(events: JsonlEvent[]): string[] {
  const errors: string[] = [];
  for (const event of events) {
...
export function classify(
  lastAssistant: LastAssistantMessage | null,
...
export function writeSnapshot(
  ...
  const diagDir = config.diagnosticsDir ?? path.join(
    os.homedir(), ".openclaw", "diagnostics", "stale-sessions",
  );
 ...
 */
export function appendDigestEntry(snapshot: StaleSnapshot, config: ForensicsConfig = {}): void {
  const diagDir = config.diagnosticsDir ?? path.join(
    os.homedir(), ".openclaw", "diagnostics", "stale-sessions",
  );
...
function redactSnapshot(snapshot: StaleSnapshot): StaleSnapshot {
  // Deep clone and redact sensitive fields
  const cloned = JSON.parse(JSON.stringify(snapshot)) as StaleSnapshot;
 ...
 */
export async function fetchLinearTicketState(
  ticketId: string,
  agentId: string,
  ...
  try {
    ...
    const body = (await res.json()) as {
      data?: { issue?: LinearIssueState | null };
...
// ── Linear workflow state resolution ─────────────────────────────────────────
...
async function fetchWorkflowStates(
  ...
  try {
    ...
    const body = (await res.json()) as {
      data?: { team?: { workflow: { states: WorkflowState[] } } | null };
    };
...
export async function recoverTicket(
  ...
  try {
    ...
    const issueBody = (await issueRes.json()) as { data?: { issue?: { id: string; team: { id: string } } | null } };
 ...
 */
function getRecoveryTargetStateName(cls: StaleClass): string | null {
 ...
 */
export function formatDigestSummary(summary: DigestSummary): string {
  const lines: string[] = [
  ...
  for (const [cls, count] of Object.entries(summary.byClass).sort((a, b) => b[1] - a[1])) {
    const name = STALE_CLASS_NAMES[cls as StaleClass] ?? cls;
