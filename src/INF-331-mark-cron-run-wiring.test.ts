...
//
// This test boots the full production entry point and polls /health until all
// registered cron drivers show a non-null lastRunAt, proving the wiring is
// genuinely live in the deployed artifact, not just importable in isolation.

describe("INF-331 [AI-1808]: production entry point stamps lastRunAt for all registered crons", () => {
  ...
  it(
    "/health.crons entries have non-null lastRunAt for every driver",
    async () => {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `entry point never responded: ${err instanceof Error ? err.message : String(err)}\n` +
              `child stderr:\n${childStderr}`,
          );
        }
        ...
        for (const name of expectedCrons) {
          ...
          expect(entry).toBeDefined();
          // FAILS on main: none of these drivers call markCronRun.
          expect(entry.lastRunAt).not.toBeNull();
          expect(typeof entry.lastRunAt).toBe("string");
        }
      } finally {
