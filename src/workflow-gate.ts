...
async function fetchBranchAndPRStatus(
  ...
  try {
    ...
    /**
     * Check Linear's attachment metadata first (fast path). If no PR shows as
     * merged in Linear, verify each PR directly via the GitHub API (INF-132).
     * Linear's GitHub integration syncs PR data asynchronously, so the metadata
     * can be stale — a merged PR may still show "open" in Linear for minutes
     * or longer. The GitHub API is the source of truth for merge state.
     */
    let hasMergedPR = prNodes.some((n) => {
    ...
    if (!hasMergedPR && hasPR) {
      // Fall through to GitHub API — Linear metadata is stale or incomplete.
      hasMergedPR = await verifyPrMergeStateViaGitHub(prNodes.map((n) => n.url!));
    }
    return {
