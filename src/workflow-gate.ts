...
export async function applyStateTransition(
  ...
  if (applied && toStateName === 'spawning' && currentStateName === 'validating' && workflowId === 'sprint') {
    try {
      ...
      log.info(
        `workflow-gate: §14b cycle counter: ${issueId} spawning cycle ${newCycleCount}`,
      );
      // If the cycle count is getting high, log a warning for observability.
      if (newCycleCount >= 3) {
        log.warn(
          `workflow-gate: §14b HIGH CYCLE COUNT: ${issueId} has re-spawned ${newCycleCount} times — sprint may be failing its own integrated AC`,
        );
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  if (matchedTransition?.feedback?.required && options?.observationStore && options?.feedback) {
    try {
      ...
      if (!validatedReason) {
        ...
        log.warn(
          `workflow-gate: P4-1: 'other' category requires free text — observation skipped for ${issueId}`,
        );
