...
function allFixtureFiles(): string[] {
  return fs.readdirSync(FIXTURES_DIR).filter((f) => f.endsWith(".yaml")).sort();
}
...
describe("AC1: Reconcile — all drifted workflow defs reconciled", () => {
  ...
  describe("canonical-dev-impl.yaml", () => {
    ...
    it("version reflects the reconciled value (>= 10, matching deployed state)", () => {
      // Current fixture has version 8. After reconciliation this should be
      // bumped to match the deployed version (>= 10 per ticket: deployed=15 fixture=10).
      expect(def.version).toBeDefined();
      // FIXME: This assertion WILL FAIL until the fixture is reconciled.
      // The deployed def is at version 15; the fixture should be at least 10.
      expect(def.version!).toBeGreaterThanOrEqual(10);
    });

    it("declares recovery_actor for orphaned-ticket delegation recovery", () => {
      // The deployed def already has recovery_actor: ai, but the fixture
      // does not. After reconciliation the fixture must include it.
      // FIXME: This assertion WILL FAIL until recovery_actor is added to the fixture.
      expect(def.recovery_actor).toBeDefined();
      expect(def.recovery_actor).toEqual("ai");
    });

    it("stakes format uses correct label namespace", () => {
      // The fixture correctly uses stakes:* label keys. The deployed def
      // drifted to risk:* — after reconciliation the deployed must be fixed
      // to match the canonical fixture. This test documents the expected format.
      expect(def.stakes).toBeDefined();
      expect(def.stakes!.levels).toBeDefined();
      // These are the correct canonical keys (stakes: namespace)
  ...
  describe("canonical-ux-audit.yaml", () => {
    ...
    it("version is >= 2 (no regression)", () => {
      // ux-audit is currently at version 2; verify it hasn't regressed.
      // FIXME: If ux-audit is also drifted (one of the 8/10), this may need updating.
      expect(def.version).toBeDefined();
  ...
  describe("canonical-sprint.yaml", () => {
    ...
    it("version is >= 1 (no regression)", () => {
      // sprint is currently at version 1; verify it hasn't regressed.
      // FIXME: If sprint is also drifted, this may need updating.
      expect(def.version).toBeDefined();
  ...
  describe("all canonical fixtures share correct structural invariants", () => {
    it("every fixture has an id, version, archetype, and entry_state", () => {
      ...
      expect(files.length).toBeGreaterThanOrEqual(3); // at minimum the three known fixtures
    ...
    it("all fixtures load cleanly through loadWorkflowRegistry with no failures", async () => {
      ...
      expect(registry.size).toBe(allFixtureFiles().length);
    });

    it("no fixture uses risk:* label format — stakes:* is the canonical namespace", () => {
      // AC1: the risk vs stakes key drift must be resolved in favour of stakes:*
      const files = allFixtureFiles();
      for (const f of files) {
        const def = parseYaml(path.join(FIXTURES_DIR, f));
...
describe("AC2: Behavioral — representative transition resolves correctly", () => {
  ...
  it("dev-impl: tests-ready in write-tests transitions to implementation", async () => {
    ...
    // Verify assign mode is `required` (dev is multi-body)
  ...
  it("dev-impl: approve in code-review transitions to deployment", async () => {
    const registry = await loadWorkflowRegistry();
    const devImpl = registry.get("dev-impl");
    expect(devImpl).toBeDefined();
    ...
    expect(approveTx!.to).toBe("deployment");
  });

  it("dev-impl: deploy transitions to ac-validate (not directly to done)", async () => {
    // AC2: after reconciliation, the full transition chain must be intact.
    // deploy (→ ac-validate) is the forward path; handoff-host-deploy is an alternate.
    const registry = await loadWorkflowRegistry();
    const devImpl = registry.get("dev-impl");
    expect(devImpl).toBeDefined();

    const deployState = devImpl!.states.find((s) => s.id === "deployment");
    expect(deployState).toBeDefined();

    const deployTx = deployState!.transitions?.find((t) => t.command === "deploy");
    expect(deployTx).toBeDefined();
    expect(deployTx!.to).toBe("ac-validate");

    const hostDeployTx = deployState!.transitions?.find((t) => t.command === "handoff-host-deploy");
    expect(hostDeployTx).toBeDefined();
    expect(hostDeployTx!.to).toBe("host-deploy");
  });
...
describe("AC3: fixture-drift warning is clean — zero drift after reconciliation", () => {
  ...
  it("all fixture files load without config-health failures", async () => {
    // After reconciliation, loadWorkflowRegistry must not record any
    // workflow-def failures in config-health. This simulates the startup
    // path and verifies it's clean.
    const registry = await loadWorkflowRegistry();
    expect(registry.size).toBe(allFixtureFiles().length);

    // Verify every def loaded without error by checking structural health
  ...
  it("no unreachable terminal states — each terminal has at least one incoming transition", async () => {
    ...
    for (const [, def] of registry) {
      ...
      for (const terminalId of terminalIds) {
        // In dev-impl, 'escape' is reachable via break_glass from every state
  ...
  it("dev-impl v8+ states match the v7+ design: write-tests, host-deploy, ac-validate present", async () => {
    const registry = await loadWorkflowRegistry();
    const devImpl = registry.get("dev-impl");
    expect(devImpl).toBeDefined();
    const stateIds = new Set(devImpl!.states.map((s) => s.id));

    // These states were added in v7/v8 design and must be present
    expect(stateIds.has("write-tests")).toBe(true);
    expect(stateIds.has("host-deploy")).toBe(true);
...
describe("AC4: Documentation — fixture header explains canonical source & regeneration", () => {
  ...
  it("README or docs mention how fixtures are regenerated from the vault", () => {
    // After reconciliation, the project documentation (or at minimum the fixture
    // headers) must explain how to regenerate fixtures when the deployed defs
    // are updated, to prevent silent re-accumulation of drift.
    ...
    if (fs.existsSync(readmePath)) {
      ...
      // One of: README documents regeneration, OR fixture headers identify canonical source.
      // After reconciliation, the README should have explicit regeneration instructions.
      expect(readmeHasRegenDoc || fixtureMentionsSync).toBe(true);
    } else {
      // No README — fixture headers must identify the canonical source
